...
public static Double estadisticas(Integer aciert, Integer fallos, Integer total, Boolean por100, Boolean asterisk_mode){
	...
	if(por100){
  		System.out.println("-Aciertos:"+aciert);
...
public static Boolean coteja(String node, String elemento ){
	...
	if(asterisk_mode){
		System.out.print(" -> Asterisl elems: "+asterisk_proteins.toString());
...
elementos :  #(OR  (p:PROTEIN {or.add(p.getText());} )+  
								{
									...
									//tama–o de la cadena de entrada.
									 ...
									 System.out.print("NODOo: OR  "+or.toString()+"\t\tENTRADA: "+elemento);
									 //Cotejamiento
									 if(asterisk_mode){
									 	last_is_or = true;
										...
										if(is_contained){
											...
											asterisk_proteins.clear();
											aciertos++;
											System.out.println("\t\t\tAcierto!: "+aciertos+"    "+ajuste_aciertos);
										}else{
											...
											asterisk_proteins.clear();
											fallos++;
											System.out.println("\t\t\tFallo!: "+fallos+"         "+ajuste_aciertos);
										}		
									 }else{
										//Si el elemento anterior no fue un *
										 if(or.contains(elemento) ){
								 			...
								 			fallos++;
								 			System.out.println("\t\t\tFallo!: "+fallos);
								 	 }
								 	//Operaciones de puesta a punto para proximas iteraciones
								 	 or.clear();
									 counter++;	
			...
			| pr:PROTEIN {		
							...
							if(counter < total ){	
								...
								elemento = elementos.get(counter);
								System.out.print("NODOs: "+node +"\t\t\tENTRADA: "+elemento );
								//Cotejamiento
								...
								System.out.println("NODOm: "+node +"\t\t\tENTRADA: "+elemento );
								if(with_asterisk){
									if(coteja(node, elemento) ){
										aciertos++;
										System.out.println("\t\t\tAcierto!: "+aciertos+"    "+ajuste_aciertos );
